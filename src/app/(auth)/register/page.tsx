  ...
  useEffect(() => {
    fetch("/api/setup/status")
      ...
      .then((data: { needsSetup: boolean }) => {
        setCanRegister(!data.needsSetup);
        if (data.needsSetup) {
    ...
    <div className="w-full max-w-sm">
      ...
      <div className="rounded-2xl border border-border bg-bg-card px-8 py-7 shadow-sm">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <LanguageSelect value={language} onChange={setLanguage} disabled={submitting} id="language-register" />
          ...
          >
            {submitting ? tAuth("creatingAccount") : tCommon("createAccount")}
          </Button>
        </form>
      </div>
